...
function playerSkillAttack(attacker, playerQueue, defendQueue, buffOper, defenderRecords) {
    ...
    var skillId = attacker.skills[attacker.fightAttri.skillPos];
    ...
    // attacker.fightAttri.skillPos = attacker.fightAttri.skillPos === 3 ? 1 : attacker.fightAttri.skillPos + 1;
    return skillId;
}
...
function calcSkilldamage(attacker, defender, attackType, oper, opt) {
    if (opt.damage > 0) {
        //固定伤害
        defender.hp = damage = defender.fightAttri.hp - damage;
    ...
    if (attackType !== 3) {
        ...
        if (Math.floor(Math.random() * 100) <= rate) {
            oper.reacOper = 2;
            console.log(defender.name + '闪避了' + attacker.name + '的攻击——————(' + attacker.name + '的hp:' + attacker.fightAttri.hp + ';' + defender.name + '的hp:' + defender.fightAttri.hp + ')');
            return;
        }
    }
    if (attackType !== 3) {
    ...
    if (damage < 0) {
        oper.damage = 0;
        console.log(defender.name + '受到' + attacker.name + '的' + oper.damage + '伤害——————(' + attacker.name + '的hp:' + attacker.fightAttri.hp + ';' + defender.name + '的hp:' + defender.fightAttri.hp + ')');
        return;
    }
    oper.damage = Math.floor(damage * (damagePer + opt.damagePer - defender.fightAttri.reduceDamagePer - opt.reduceDamagePer) / 100);
    defender.fightAttri.hp = defender.fightAttri.hp - oper.damage;
    if (attacker.vampirePer > 0 && attackType !== 3 && attacker.status !== 1) {
        hp = attacker.fightAttri.hp + oper.damage * attacker.vampirePer / 100;
        oper.vampireHurt = hp < attacker.fightAttri.maxHp ? hp : attacker.fightAttri.maxHp;
        attacker.fightAttri.hp = attacker.fightAttri.hp + oper.vampireHurt;
    }
    console.log(defender.name + '受到' + attacker.name + '的' + oper.damage + '伤害——————(' + attacker.name + '的hp:' + attacker.fightAttri.hp + ';' + defender.name + '的hp:' + defender.fightAttri.hp + ')');
    return;
};
...
function battling(attackQueue, playerQueue, defendQueue, round, buffOper, fightRecord) {
    ...
    while (attackQueue.length > 0) {
        ...
        var attackerRecord = {
            hp: attacker.fightAttri.hp,
            pos: attacker.fightAttri.pos,
            skillId: skillId,
            name: attacker.name,
        ...
        for (var i = length - 1; i >= 0; i--) {
            ...
            if (gBuffOper[i].tick === 0) {
                ...
                var buff = {
                    ...
                    pos: gBuffOper[i].player.fightAttri.pos,
                    round: round
                }
                buffs.push(buff);
            }
...
exports.startBattle2 = function(fightStructTmp1 , fightStructTmp2) {
    //打怪
    gBuffOper = [];
    fightRecord = {
        roundCount: 0,
    ...
    for (var i in fightStructTmp2) {
        ...
        player.fightAttri.pos = i;
        exptotal = fightStructTmp2[i].getexp;
        console.log(fightStructTmp2[i].getexp);
        var player = fightStructTmp2[i];
        player.fightAttri.pos = i;
        fightRecord.queue2.push(player);
    }
    ...
    while (defendQueue1.length > 0 && defendQueue2.length > 0) {
        ...
        fightRecord.buffs.push(buffs);
        round++;
        if (defendQueue2.length === 0) {
            results = [];
            var length = fightStructTmp1.length
            for (var i in fightStructTmp1) {
                var exp = Math.floor(exptotal / length);
                fightStructTmp1[i].exp += exp;
                var result = {
                    pos: fightStructTmp1[i].fightAttri.pos,
                    exp: exp
                }
                results.push(result)
            }
            fightRecord.results = results;
            break;
        }
    }
    fightRecord.roundCount = round;
